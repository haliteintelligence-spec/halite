  ...
  useEffect(() => {
    async function load() {
      const [brandRes, eventsRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/brands/${brandId}`, {
          headers: token() ? { Authorization: `Bearer ${token()}` } : {},
          cache: 'no-store',
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/brands/${brandId}/login-events`, {
          headers: token() ? { Authorization: `Bearer ${token()}` } : {},
          cache: 'no-store',
        }),
      ])
      if (brandRes.ok) {
        const data = await brandRes.json() as { brand: { name: string } }
